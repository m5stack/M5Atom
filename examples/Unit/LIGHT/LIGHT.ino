...
* date：2021/8/18
*******************************************************************************
  Please connect to Port,Use the Light Unit Serial to display the current ambient lighting value
  请连接端口 ,使用Light Unit 串口显示当前环境光照值。
*/
...
void loop() {
  ...
  analogRead_value = analogRead(32);  //Store the analog quantity read from pin 32.  将32号引脚读取到的模拟量存储
